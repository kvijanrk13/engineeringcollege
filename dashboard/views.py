...
def faculty_dashboard(request):
    """Single page faculty dashboard - combines all faculty functionality"""
    ...
    subjects = faculty.subjects_dealt.split(',') if faculty.subjects_dealt else []
...
def generate_exam_branch_report(request):
    ...
    if request.GET.get('format') == 'pdf':
        ...
        try:
            ...
            messages.error(request, f"Excel export requires pandas: {str(e)}")
            return redirect('dashboard:generate_exam_branch_report')
        except Exception as e:
            return JsonResponse({'success': False, 'error': str(e)})
        ...
        try:
            ...
            if isinstance(pdf_response, HttpResponse):
                ...
                return JsonResponse({
                    ...
                    'download_url': reverse('dashboard:download_faculty_pdf', args=[faculty_id])
                })
            else:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                messages.error(request, f'Pandas is required for importing: {str(e)}')
            except Exception as e:
...
def download_faculty_pdf_by_code(request, employee_code):
    ...
    try:
        faculty = Faculty.objects.get(employee_code=employee_code.upper())
        return redirect('dashboard:download_faculty_pdf', faculty_id=faculty.id)
    except Faculty.DoesNotExist:
...
def filter_faculty(request):
    ...
    if pdf_available == 'yes':
        ...
        faculties = Faculty.objects.exclude(cloudinary_pdf_url='').filter(**filters)
    elif pdf_available == 'no':
        filters['cloudinary_pdf_url__isnull'] = True
        faculties = Faculty.objects.filter(**filters)
    else:
        faculties = Faculty.objects.filter(**filters)
    ...
    faculties = faculties.order_by('staff_name')
...
def generate_faculty_pdf_bytes(faculty):
    ...
    Prevents views.py import failure
    """
    return b""  # return empty PDF bytes safely


def merge_certificates_with_pdf_bytes(pdf_bytes, faculty):
    """
    TEMPORARY SAFE IMPLEMENTATION
    Returns original PDF without crashing
    """
    return pdf_bytes
...
def get_disk_usage():
    ...
    try:
        ...
        return {'error': 'Unable to get disk usage', 'message': str(e)}
...
def get_memory_usage():
    ...
    try:
        ...
        return {'error': 'Unable to get memory usage', 'message': str(e)}
            ...
            if action == 'update':
                ...
                for api_field, model_field in field_map.items():
                    if api_field in data:
                        setattr(faculty, model_field, data[api_field])

                # Handle date fields separately
                if 'joining_date' in data:
                    faculty.joining_date = data['joining_date']
                if 'dob' in data:
...
def faculty_qrcode(request, faculty_id):
    ...
    try:
        ...
        qr_data = {
            ...
            'email': faculty.email,
            'mobile': faculty.mobile,
            'profile_url': request.build_absolute_uri(reverse('dashboard:faculty', args=[faculty.id])),
            'pdf_url': faculty.cloudinary_pdf_url,
        }
