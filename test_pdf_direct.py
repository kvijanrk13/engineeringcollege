...
def generate_pdf_direct(faculty_id=7001):
    try:
        faculty = Faculty.objects.get(employee_code=str(faculty_id))
        
        print(f"\n{'='*60}")
        print(f"DIRECT PDF GENERATION FOR: {faculty.staff_name}")
        print(f"{'='*60}\n")
        ...
        html_file = f"direct_test_{faculty.employee_code}.html"
        ...
        if os.path.exists(wkhtml_path):
            ...
            pdf_file = f"direct_test_{faculty.employee_code}.pdf"
            with open(pdf_file, 'wb') as f:
                f.write(pdf)
            print(f"✓ PDF saved to: {pdf_file}")
            
            # Also save a copy as faculty_{code}.pdf for easy access
            final_file = f"faculty_{faculty.employee_code}.pdf"
            with open(final_file, 'wb') as f:
                f.write(pdf)
            print(f"✓ Also saved as: {final_file}")
            
            # Print the full path
            print(f"\nPDF Location: {os.path.abspath(final_file)}")
        else:
