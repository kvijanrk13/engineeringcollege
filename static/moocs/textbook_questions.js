...
const PYQ_2017_QUESTIONS=[
Q('Set 3','Operating Systems','Using SRT scheduling for P1(20,0), P2(25,15), P3(10,30), and P4(15,45), where each pair is (execution time, arrival time), what is P2’s total waiting time?',['5','15','40','55'],1,'P2 runs from 20–30, is preempted by P3, then finishes at 55. Waiting time = (55−15)−25 = 15.'),
Q('Set 3','File Systems','Data blocks of a very large file in the UNIX file system are allocated using:',['Contiguous allocation','Linked allocation','Single indexed allocation only','An extension of indexed allocation'],3,'UNIX inodes extend indexed allocation with direct and multiple levels of indirect block pointers.'),
Q('Set 3','System Calls','Match chmod, fork, exec, creat respectively with their functions.',['executes, creates file, changes permission, creates child','changes permission, creates child, executes, creates file','creates file, changes permission, creates child, executes','creates child, executes, creates file, changes permission'],1,'chmod changes permissions, fork creates a child, exec executes a program, and creat creates a file.'),
Q('Set 3','System Calls','Which system call is not related to the UNIX file system?',['creat','close','stat','wait'],3,'wait concerns process synchronization; the others operate on file-system objects.'),
Q('Set 3','Operating Systems','If t1 is a user/kernel mode-switch time and t2 is a process context-switch time, which relation normally holds?',['t1 > t2','t1 = t2','t1 < t2','No relation is possible'],2,'A process switch saves and restores more context, so it normally costs more than a mode switch.'),
Q('Set 3','Relations','For R={(1,2),(2,3),(3,4),(4,2)} and S={(1,3),(2,4),(4,2),(4,3)}, what is S∘R when S is applied first?',['{(1,1),(2,3),(4,3),(4,4)}','{(1,3),(2,3),(3,4),(4,4)}','{(1,1),(2,2),(4,3),(4,4)}','{(1,4),(2,2),(4,3),(4,4)}'],3,'Following each S edge by an R edge gives (1,4), (2,2), (4,3), and (4,4).'),
Q('Set 3','Relations','How many different relations exist from an m-element set to an n-element set?',['2^m','mn','2^(mn)','2^n'],2,'A relation is any subset of the mn-element Cartesian product, giving 2^(mn) subsets.'),
Q('Set 3','Combinatorics','How many integers from 1 through 10^4−1 contain exactly one digit 8 and one digit 9 when written as four positions with leading zeros?',['2×3×8²','4×3×8²','8×9×8²','8×3×9²'],1,'Choose the position of 8 in 4 ways, 9 in 3 ways, and each remaining digit from 8 allowed digits.'),
Q('Set 3','Formal Languages','The language {a^n b^n c^n : n≥1} is:',['Context-sensitive but not context-free','Context-free but not regular','Regular but not context-sensitive','Context-free and context-sensitive only'],0,'Equal counts across three ordered blocks require context-sensitive power and are not context-free.'),
Q('Set 3','Graph Theory','A path containing every vertex exactly once is a:',['Euler tour','Hamiltonian path','Euler trail','Hamiltonian tour'],1,'A Hamiltonian path visits every vertex exactly once.'),
Q('Set 3','Networking','Which protocol is connection-oriented?',['TCP','IP','SNMP','DNS'],0,'TCP establishes a logical connection and supplies reliable ordered delivery.'),
Q('Set 3','Networking','The Internet predominantly uses:',['Circuit switching','Packet switching','Streaming switching','Automatic switching'],1,'Internet traffic is divided into independently routed packets.'),
Q('Set 3','Subnetting','A class-B host address split with a 6-bit subnet number supports, under the traditional rule:',['62 subnets and 262142 hosts','64 subnets and 262142 hosts','62 subnets and 1022 hosts','64 subnets and 1024 hosts'],2,'Traditional subnetting gives 2^6−2=62 subnets and 2^10−2=1022 hosts per subnet.'),
Q('Set 3','IP','Why does an IP datagram contain a Time To Live field?',['To prioritize packets','To reduce propagation delay','To optimize throughput','To discard packets that loop too long'],3,'Routers decrement TTL; expiry removes a datagram instead of allowing an endless routing loop.'),
Q('Set 3','Transport Protocols','Protocols commonly used for real-time multimedia, file transfer, DNS, and email respectively are:',['TCP, UDP, UDP, TCP','UDP, TCP, TCP, UDP','UDP, TCP, UDP, TCP','TCP, UDP, TCP, UDP'],2,'Real-time media and DNS commonly use UDP, while file transfer and email use TCP.'),
Q('Set 3','Database Keys','For R(A,B,C,D,E) with AB→E and D→C, which listed attribute set is a candidate key?',['ABC','ABCDE','ABCE','ABD'],3,'ABD determines E through AB and C through D, and includes all attributes not derivable from others.'),
Q('Set 3','Normalization','Which statement about normal forms is false?',['BCNF addresses FD redundancy','FD-based normal forms are nested','3NF eliminates redundancy from multivalued dependencies','3NF aims to preserve functional dependencies'],2,'Multivalued dependencies are addressed by fourth normal form, not third normal form.'),
Q('Set 3','Transactions','Which is not a standard SQL isolation level?',['Read committed','Repeatable read','Read uncommitted','Read-write committed'],3,'The standard levels include read uncommitted, read committed, repeatable read, and serializable.'),
Q('Set 3','Concurrency Control','The claim “two-phase locking produces optimal schedules” is:',['True for all schedules','True for some schedules','False as a general guarantee','Equivalent to serial execution'],2,'2PL guarantees conflict serializability, not optimality of the resulting schedule.'),
Q('Set 3','ER Modeling','Which cardinalities may a ternary relationship have?',['1:1:1','M:N:P','1:M:N','All of these'],3,'Ternary relationships can use one or many participation on each of their three sides.'),
Q('Set 3','Assemblers','A conventional assembler commonly resolves symbols and emits machine instructions using:',['One pass only','Two passes','Three passes only','No passes'],1,'A two-pass assembler first builds the symbol table and then generates object code.'),
Q('Set 3','Linkers','A linker combines object modules, incorporates library modules, and resolves external symbols. Which option is correct?',['Only the first function','Only library combination','Only symbol resolution','All of these functions'],3,'All three are standard linker responsibilities.'),
Q('Set 3','Language Translation','A high-level program can be processed for execution by:',['An interpreter','A compiler','An assembler directly','Either an interpreter or compiler'],3,'Both interpreters and compilers process high-level languages; assemblers process assembly language.'),
Q('Set 3','Intermediate Code','Which is not an intermediate-code representation?',['Quadruples','Triples','Machine code','P-code'],2,'Machine code is target code, whereas the others can serve as intermediate representations.'),
Q('Set 3','Compiler Phases','Correctly match lexical analysis, syntax analysis, code generation, and optimization.',['grammar, strength reduction, lexemes, machine code','grammar, lexemes, strength reduction, machine code','lexemes, grammar, machine code, strength reduction','machine code, lexemes, strength reduction, grammar'],2,'Lexing creates lexemes/tokens, parsing checks grammar, code generation emits machine code, and optimization performs strength reduction.'),
Q('Set 3','Requirements','The description of a product or program and the functions it must perform in its target environment is the:',['Software Requirements Specification','Software Design Document','Software Test Plan','Risk Management Plan'],0,'An SRS defines required behavior, interfaces, constraints, and environment.'),
Q('Set 3','Software Metrics','A metric derived empirically from countable information-domain measures and used to assess software size/complexity is:',['Coupling','Cohesion','Coupling between classes','Function point'],3,'Function points use weighted counts of inputs, outputs, inquiries, files, and interfaces.'),
Q('Set 3','Structured Design','Transform or transaction mapping of a data-flow diagram ultimately produces a:',['Context-level DFD','Second-level DFD','Structure chart','Architecture diagram only'],2,'Structured design maps data flow into a module hierarchy represented by a structure chart.'),
Q('Set 3','Software Testing','Re-executing previously run tests to detect unintended effects of changes is:',['Stress testing','Performance testing','Regression testing','Glass-box testing'],2,'Regression testing checks that changes have not broken existing behavior.'),
Q('Set 3','Boundary Testing','Under robust boundary-value analysis, how many tests are used for four input variables?',['16','64','256','25'],3,'Robust BVA uses 6n+1 tests; for n=4 this is 25.'),
Q('Set 3','Logic','From P∨Q, Q→R, P→M, and ¬M, which conclusion follows?',['R∨(P∧Q)','P∧(Q∨R)','P∨(Q∧R)','R∧(P∨Q)'],3,'Modus tollens gives ¬P, hence Q and then R; therefore R∧(P∨Q).'),
Q('Set 3','Flip-Flops','An SR flip-flop becomes which flip-flop when one input is inverted and applied to S and R?',['Master-slave','T','JK','D'],3,'Driving S=D and R=¬D produces the characteristic behavior of a D flip-flop.'),
Q('Set 3','Number Systems','What is the 7’s complement of octal 65402?',['12375','12376','11375','11376'],0,'Subtract each digit from 7: 7−6, 7−5, 7−4, 7−0, 7−2 gives 12375.'),
Q('Set 3','Registers','A general-purpose shift register may have a clock, n parallel outputs, and shift-left serial input. Which choice is correct?',['Clock only','Parallel outputs only','Shift-left support only','All of these'],3,'A universal/general shift register combines these control and data capabilities.'),
Q('Set 3','Number Conversion','Which statement about hexadecimal 2FA0C is correct?',['It equals decimal 195084','It equals binary 101111101000001100','Both statements are correct','It equals octal 375014'],2,'2FA0C₁₆ equals 195084₁₀ and 101111101000001100₂.'),
Q('Set 3','C Declarations','What does int (*P[10])() declare?',['A pointer to one function','An array of 10 pointers to functions returning int','A function returning an array','A pointer to an array of functions'],1,'Brackets bind P as an array; each element is a pointer to a function returning int.'),
Q('Set 3','Object-Oriented Programming','Which statement illustrates polymorphism?',['Fields must be private','A class extends another class','A supertype variable refers to a subtype object','A class contains another class'],2,'Subtype objects can be used through a supertype reference and dispatch overridden behavior dynamically.'),
Q('Set 3','Recursion','A factorial function recursively calls factorial(a+1) whenever a>1. Calling factorial(3) results in:',['6','24','18','Unbounded recursion and eventual stack failure'],3,'The argument increases, so the base case is never reached.'),
Q('Set 3','Bitwise Operators','If X=63 and Y=30, what is X XOR Y?',['47','40','65','33'],3,'00111111 XOR 00011110 equals 00100001, which is decimal 33.'),
Q('Set 3','Recursion','A function func(n) first calls func(n−1) when n>1, then prints # exactly n times. How many # characters does func(5) print?',['15','60','10','20'],0,'The total is 1+2+3+4+5=15.'),
Q('Set 3','Trees','With the root at level 0, the maximum number of nodes at level i of a binary tree is:',['2^(i−1)','2^i−1','2^i','2^(i+1)'],2,'Each level can double the preceding level, yielding at most 2^i nodes.'),
Q('Set 3','Graph Theory','Every connected undirected graph with n vertices contains:',['n−2 edges','At most n−2 edges','Exactly n−1 edges','At least n−1 edges'],3,'A spanning tree already requires n−1 edges; a connected graph may contain more.'),
