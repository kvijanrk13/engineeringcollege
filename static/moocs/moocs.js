...
const $=id=>document.getElementById(id);const EXAM_CONFIG=Object.fromEntries(Array.from({length:20},(_,i)=>[i+1,{minutes:60,marks:200}]));let current=0,seconds=3600,timer=null,selectedSet=1,examInProgress=false,allowNavigation=false,state=QUESTIONS.map(()=>({answer:null,visited:false,review:false}));
...
// Guide candidates through all twenty 100-question papers in order.
const nextSetPanel=$('next-set-panel'),continueNextSet=$('continue-next-set');
const savedProgress=readProgress();
const initialSet=savedProgress.active?.set||Math.min(20,Math.max(1,Math.max(0,...(savedProgress.completed||[]))+1));
...
const updateNextSetPrompt=()=>{
  ...
  const hasNext=selectedSet>=1&&selectedSet<20;
