...
class ParentContactForm(forms.ModelForm):
    class Meta:
        model = ParentContact
        fields = ['student', 'parent_name', 'phone_number', 'email', 'preferred_channel']
        widgets = {
            'student': forms.Select(attrs={'class': 'form-select'}),
            'parent_name': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Parent or guardian name'}),
            ...
            'preferred_channel': forms.Select(attrs={'class': 'form-select'}),
        }


class ParentAbsenceSearchForm(forms.Form):
    lecture_date = forms.DateField(
        widget=forms.DateInput(attrs={'class': 'form-control', 'type': 'date'})
    )
    subject = forms.ModelChoiceField(
...
class ParentAcknowledgementForm(forms.ModelForm):
    class Meta:
        model = ParentNotification
        fields = ['acknowledgement_reason', 'acknowledgement_note']
        widgets = {
            'acknowledgement_reason': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Reason, e.g. Medical leave'}),
            'acknowledgement_note': forms.Textarea(attrs={'class': 'form-control', 'rows': 3}),
        }
