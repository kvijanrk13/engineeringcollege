...
from .forms import (
    ...
    BulkAttendanceForm,
    DepartmentForm,
    ParentAbsenceSearchForm,
    ParentAcknowledgementForm,
    ParentContactForm,
    StudentForm,
    SubjectForm,
)
...
def _parent_notification_recipient(parent_contact):
    if parent_contact.preferred_channel == ParentContact.CHANNEL_EMAIL:
        return parent_contact.email
    return parent_contact.phone_number


def _build_parent_absence_message(attendance):
    subject_name = attendance.subject.title if attendance.subject else 'class'
    return (
        f"Dear Parent, your ward {attendance.student.full_name} "
        f"was marked absent for {subject_name} on {attendance.lecture_date}. "
        "Please acknowledge this message or contact the class teacher if correction is needed."
    )


def _send_parent_notification(notification):
    try:
        if notification.channel == ParentContact.CHANNEL_EMAIL:
            send_mail(
                'CloudAttend Absence Alert',
                notification.message,
                getattr(settings, 'DEFAULT_FROM_EMAIL', 'cloudattend@example.com'),
                [notification.recipient],
                fail_silently=False,
            )
        else:
            # Replace this console output with Twilio, AWS SNS, MSG91, or WhatsApp API calls.
            print(f"{notification.channel.upper()} to {notification.recipient}: {notification.message}")
        notification.status = ParentNotification.STATUS_SENT
        notification.sent_at = timezone.now()
        notification.error_message = ''
    except Exception as exc:
        notification.status = ParentNotification.STATUS_FAILED
...
@login_required
def parent_contact_list(request):
    guard = _require_admin(request)
    if guard:
        return guard
    contacts = ParentContact.objects.select_related('student').order_by('student__first_name')
    return render(request, 'parents/list.html', {'contacts': contacts})
...
def parent_contact_create(request):
    ...
    if guard:
        return guard
    form = ParentContactForm(request.POST or None)
    if request.method == 'POST' and form.is_valid():
        form.save()
        messages.success(request, 'Parent contact saved.')
        return redirect('parent_contact_list')
    return render(request, 'parents/form.html', {'form': form, 'title': 'Add Parent Contact'})


@login_required
def parent_absence_notifications(request):
    guard = _require_admin(request)
    if guard:
        return guard

    form = ParentAbsenceSearchForm(request.GET or None)
    absences = Attendance.objects.none()
    if form.is_valid():
        absences = Attendance.objects.select_related(
            'student', 'student__department', 'student__parent_contact', 'subject'
        ).filter(lecture_date=form.cleaned_data['lecture_date'], status='Absent')
        if form.cleaned_data.get('subject'):
            absences = absences.filter(subject=form.cleaned_data['subject'])
        if form.cleaned_data.get('department'):
            absences = absences.filter(student__department=form.cleaned_data['department'])

    if request.method == 'POST':
        form = ParentAbsenceSearchForm(request.POST)
        if form.is_valid():
            absences = Attendance.objects.select_related(
                'student', 'student__parent_contact', 'subject'
            ).filter(lecture_date=form.cleaned_data['lecture_date'], status='Absent')
            if form.cleaned_data.get('subject'):
                absences = absences.filter(subject=form.cleaned_data['subject'])
            if form.cleaned_data.get('department'):
            ...
            for attendance in absences:
                parent_contact = getattr(attendance.student, 'parent_contact', None)
                if not parent_contact:
                    skipped_count += 1
                    continue
                recipient = _parent_notification_recipient(parent_contact)
                if not recipient:
                    skipped_count += 1
                    continue
                notification, created = ParentNotification.objects.get_or_create(
                    ...
                    defaults={
                        'channel': parent_contact.preferred_channel,
                        'recipient': recipient,
                        'message': _build_parent_absence_message(attendance),
                ...
                if request.POST.get('send_now') == '1':
                    _send_parent_notification(notification)
            messages.success(
                request,
                f'{created_count} notification(s) prepared. {skipped_count} absence record(s) skipped.',
            )
            return redirect('parent_notification_logs')

    return render(request, 'notifications/absent_parents.html', {'form': form, 'absences': absences})


@login_required
def parent_notification_logs(request):
    guard = _require_admin(request)
    if guard:
        return guard
    logs = ParentNotification.objects.select_related(
        'attendance', 'attendance__student', 'parent_contact'
    )[:100]
    return render(request, 'notifications/logs.html', {'logs': logs})


@login_required
def parent_notification_send(request, pk):
    guard = _require_admin(request)
    if guard:
        return guard
    notification = get_object_or_404(ParentNotification, pk=pk)
    _send_parent_notification(notification)
    messages.success(request, 'Parent notification processed.')
    return redirect('parent_notification_logs')


def parent_acknowledge(request, pk):
    notification = get_object_or_404(ParentNotification, pk=pk)
    form = ParentAcknowledgementForm(request.POST or None, instance=notification)
    if request.method == 'POST' and form.is_valid():
        acknowledgement = form.save(commit=False)
        acknowledgement.status = ParentNotification.STATUS_ACKNOWLEDGED
        acknowledgement.acknowledged_at = timezone.now()
        acknowledgement.save()
        return render(request, 'notifications/acknowledged.html', {'notification': acknowledgement})
    return render(request, 'notifications/acknowledge.html', {'form': form, 'notification': notification})
