...
class ParentContact(TimeStampedModel):
    CHANNEL_EMAIL = 'email'
    CHANNEL_SMS = 'sms'
    ...
    student = models.OneToOneField(
        Student, on_delete=models.CASCADE, related_name='parent_contact'
    )
    parent_name = models.CharField(max_length=160)
    phone_number = models.CharField(max_length=20, blank=True)
    email = models.EmailField(blank=True)
    preferred_channel = models.CharField(
        max_length=20, choices=CHANNEL_CHOICES, default=CHANNEL_EMAIL
    )

    def __str__(self):
        return f"{self.parent_name} ({self.student.full_name})"
...
class ParentNotification(TimeStampedModel):
    ...
    attendance = models.ForeignKey(
        Attendance, on_delete=models.CASCADE, related_name='parent_notifications'
    )
    parent_contact = models.ForeignKey(
        ParentContact, on_delete=models.CASCADE, related_name='notifications'
    )
    ...
    acknowledgement_reason = models.CharField(max_length=120, blank=True)
    ...
    def __str__(self):
        return f"{self.parent_contact.parent_name} | {self.attendance.student.full_name} | {self.status}"
